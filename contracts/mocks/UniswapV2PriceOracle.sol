...
/* ==========  Constants  ========== */
  ...
  uint32 public MAXIMUM_OBSERVATION_AGE;
...
/* ==========  Storage  ========== */
  ...
  function setMaximumObservationAge(uint32 maxObservationAge) public {
    MAXIMUM_OBSERVATION_AGE = maxObservationAge;
  }
...
/* ==========  Price Updates  ========== */
  ...
  function updatePrice(address token) public returns (bool didUpdate) {
    Prices.PriceObservation memory _new = _uniswapFactory.observeTwoWayPrice(token, _weth);
    ...
    emit PriceUpdated(token, _new.priceCumulativeLast, _new.ethPriceCumulativeLast);
    return true;
  }
...
/* ==========  Internal Observation Functions  ========== */
    ...
    returns (Prices.PriceObservation memory observation)
  {
    ...
    // uint256 maxAge = MAXIMUM_OBSERVATION_AGE;
    ...
    bool usable = observation.timestamp != 0 && timeElapsed >= MINIMUM_OBSERVATION_DELAY;
    while(!usable) {
      observation = _priceObservations[token][--observationIndex];
      uint256 obsTime = observation.timestamp;
      timeElapsed = timestamp - (obsTime == 0 ? OBSERVATION_PERIOD * observationIndex : obsTime);
      usable = observation.timestamp != 0 && timeElapsed >= MINIMUM_OBSERVATION_DELAY;
      require(
        timeElapsed <= MAXIMUM_OBSERVATION_AGE,
